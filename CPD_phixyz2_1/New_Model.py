...
r_shifted = np.sqrt(r_masked**2 + r_mid**2 - 2*r_masked*r_mid*np.cos(pp_masked-np.pi))
...
iphi = np.argmin(np.abs(phi_c - np.pi))
print(f'ir = {ir}, iphi = {iphi}')
pplanet = [-r_c[ir], 0.0, 0,0]  # Cell center coordinates
...
plt.figure(figsize=(10, 6))
plt.plot(rr_slice / au, sigmad_slice[:, 0], label='phi = 0')
