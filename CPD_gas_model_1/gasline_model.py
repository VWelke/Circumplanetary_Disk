# reference radmc3d example code run_lines_lte_0d_1 and 
# https://github.com/dullemond/radmc3d-2.0/tree/master/examples/run_lines_lte_0d_1
# Find the molecules in LAMDA:  http://home.strw.leidenuniv.nl/~moldata/
#
...
with open('gas_velocity.inp','w+') as f:
    f.write('1\n')                       # Format number
    f.write('%d\n'%(nr*ntheta*nphi))     # Nr of cells
    for iphi in range(nphi):  # velocity at r, theta, phi direction for each grid point
        for itheta in range(ntheta):
            for ir in range(nr):
                f.write('%13.6e %13.6e %13.6e\n'%(vr[ir,itheta,iphi],vtheta[ir,itheta,iphi],vphi[ir,itheta,iphi]))
# vr[ir,itheta,iphi] : the radial component of the velocity at the grid point (ir, itheta, iphi)
...
with open('microturbulence.inp','w+') as f:
    ...
    data.tofile(f, sep='\n', format="%13.6e") #writes the flattened microturbulence data to the file. each value separated by a new line
    f.write('\n') # writes a new line
#
...
with open('lines.inp','w') as f:
    ...
    f.write('co    leiden    0    0\n')  # why seems like 1 zero is missing
#  CO (and thus read from the file molecule_co.inp), and the data format is the Leiden database format.
